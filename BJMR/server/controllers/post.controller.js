...
const getCountryByIP = require("../utils/iptocountry");
...
const getPost = async (req, res) => {
  try {
    ...
    // Retrieve the IP address from the request and enrich the post with country information
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    const country = await getCountryByIP(ip);
    post = { ...post, country }; // Add country information to the post object for display
...
const getPosts = async (req, res) => {
  try {
    ...
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress; // Get the IP address from the request
    const country = await getCountryByIP(ip); // Fetch country based on the IP
...
const getCommunityPosts = async (req, res) => {
  try {
    ...
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    // Get the country using the IP address for display purposes
    const country = await getCountryByIP(ip);
...
const getPublicPosts = async (req, res) => {
  try {
    ...
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    const country = await getCountryByIP(ip); // Assuming getCountryByIP function is defined
